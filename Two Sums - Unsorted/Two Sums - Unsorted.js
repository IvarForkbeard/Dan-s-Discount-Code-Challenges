...
const fs = require("fs");
const arrayNums = fs.readFileSync("data.txt", "utf-8").split(",");
...
function findThePair(input, target) {
    ...
    for (let i = 0; i < input.length; i++) {
        if (Number.isInteger(object[target - input[i]]) && object[target - input[i]] !== i) {
...
console.log("Output: [" + findThePair(arrayNums, desiredTarget) + "]");
